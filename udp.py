...
def client(server_port):
    s =  socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    message = str(input('Enter a lowercase sentence: '))
    data = message.encode('ascii')
    s.sendto(data, ('127.0.0.1', server_port))
...
if __name__ == '__main__':
    ...
    parser.add_argument('mode', choices=funcs.keys(), type=str, help='choose to run in client or server modes')
    parser.add_argument('-p', '--port', metavar='PORT', type=int, default=3000, help='UDP port (default 3000)')
    args = parser.parse_args()
    function = funcs[args.mode]
    function(args.port)